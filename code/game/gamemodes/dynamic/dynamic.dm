...
/*
	...
	* Hook up procs in `mob_lists.dm`
	* Implement var/list/dynamic_forced_occupations on SSjobs
	* Convert json in /datum/game_mode/dynamic/pre_setup() to DM vars (dynamic_configuration.dm) to allow normal contribs to change these settings?
			OR just use the JSON method TG uses.
...
/datum/game_mode/dynamic/proc/send_intercept()
	...
	// TODO: probably don't need this, we don't even have SScomunications
...
/datum/game_mode/dynamic/pre_setup()
	if(GLOB.configuration.dynamic.dynamic_config_enabled)
		var/json_file = file("[GLOB.config.directory]/dynamic.json")
