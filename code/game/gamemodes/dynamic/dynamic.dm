...
/*
	...
	* make all the global lists work `alive_antagonists`, etc. (or just have helper procs to search for these people, get_alive_antagonists() and so on)
	* implement var/list/dynamic_forced_occupations on SSjobs
	* convert json crap in /datum/game_mode/dynamic/pre_setup() to DM vars (dynamic_configuration.dm)
	* make setup_hijacking() work
	* test /datum/dynamic_ruleset/midround/malf/execute()
	* make /datum/dynamic_ruleset/midround/from_ghosts/proc/generate_ruleset_body() work
	* make code in dynamic_hijacking.dm work with our event system
	* make code in send_intercept() work
	* make malf AI antag datum
*/
...
/datum/game_mode/dynamic
	...
	/// List of minds that have been picked as antag by the gamemode. Removed as antag datums are set.
	var/list/pre_setup_antags = list()
	...
	var/shown_threat
	/// world.time of the next midround rule injection.
	VAR_PRIVATE/next_midround_injection

/datum/game_mode/dynamic/admin_panel()
	var/list/dat = list("<html><head><meta http-equiv='Content-Type' content='text/html; charset=UTF-8'><title>Game Mode Panel</title></head><body><h1><B>Game Mode Panel</B></h1>")
	dat += "Dynamic Mode <a href='?_src_=vars;Vars=[UID()]'>\[VV\]</a> <a href='?src=[UID()]'>\[Refresh\]</a><BR>"
	dat += "Threat Level: <b>[threat_level]</b><br/>"
	dat += "Budgets (Roundstart/Midrounds): <b>[initial_round_start_budget]/[threat_level - initial_round_start_budget]</b><br/>"

	dat += "Midround budget to spend: <b>[mid_round_budget]</b> <a href='?src=[UID()];adjustthreat=1'>\[Adjust\]</A> <a href='?src=[UID()];threatlog=1'>\[View Log\]</a><br/>"
	dat += "<br/>"
	...
	dat += "Forced extended: <a href='?src=[UID()];forced_extended=1'><b>[GLOB.dynamic_forced_extended ? "On" : "Off"]</b></a><br/>"
	dat += "No stacking (only one round-ender): <a href='?src=[UID()];no_stacking=1'><b>[GLOB.dynamic_no_stacking ? "On" : "Off"]</b></a><br/>"
	dat += "Stacking limit: [GLOB.dynamic_stacking_limit] <a href='?src=[UID()];stacking_limit=1'>\[Adjust\]</A>"
	dat += "<br/>"
	dat += "<A href='?src=[UID()];force_latejoin_rule=1'>\[Force Next Latejoin Ruleset\]</A><br>"
	if(forced_latejoin_rule)
		dat += {"<A href='?src=[UID()];clear_forced_latejoin=1'>-> [forced_latejoin_rule.name] <-</A><br>"}
	dat += "<A href='?src=[UID()];force_midround_rule=1'>\[Execute Midround Ruleset\]</A><br>"
	dat += "<br />"
	dat += "Executed rulesets: "
	if(length(executed_rules) > 0)
		dat += "<br/>"
	...
	dat += "Latejoin: [DisplayTimeText(latejoin_injection_cooldown-world.time)] <a href='?src=[UID()];injectlate=1'>\[Now!\]</a><BR>"

	var/next_injection = next_midround_injection()
	if (next_injection == INFINITY)
		dat += "All midrounds have been exhausted."
	else
		dat += "Midround: [DisplayTimeText(next_injection - world.time)] <a href='?src=[UID()];injectmid=1'>\[Now!\]</a><BR>"
...
/datum/game_mode/dynamic/proc/send_intercept()
	. = "<b><i>Nanotrasen Department of Intelligence Threat Advisory, Spinward Sector, TCD [time2text(world.realtime, "DDD, MMM DD")], [GLOB.game_year]:</i></b><hr>"
	switch(round(shown_threat))
		if(0 to 19)
			var/show_core_territory = (length(GLOB.current_living_antags) > 0)
	...
	// probably don't need this, we don't even have SScomunications
	...
	if(greenshift)
		GLOB.command_announcer.autosay("Thanks to the tireless efforts of our security and intelligence divisions, there are currently no credible threats to [station_name()]. All station construction projects have been authorized. Have a secure shift!", "Security Report")
		return

	// TODO: test announcements
	GLOB.command_announcer.autosay("A summary has been copied and printed to all communications consoles.", "Security Level Elevated")
...
/datum/game_mode/dynamic/proc/generate_threat()
	...
	if(SSticker.total_players_ready < low_pop_player_threshold)
		threat_level = min(threat_level, LERP(low_pop_maximum_threat, max_threat_level, SSticker.total_players_ready / low_pop_player_threshold))
...
/datum/game_mode/dynamic/pre_setup()
	// TODO: make just vars on dynamic, not json I think. We can just code weight onto dynamic rules and have people PR if they want to change.

	// if(GLOB.configuration.dynamic.dynamic_config_enabled)
	...
	for(var/i in GLOB.new_player_list)
		...
		if(player.mind && player.ready && player.check_preferences())
			roundstart_pop_ready++
			candidates.Add(player)
	log_dynamic("Listing [length(roundstart_rules)] round start rulesets, and [length(candidates)] players ready.")
	if(length(candidates) <= 0)
		log_dynamic("[length(candidates)] candidates.")
		return TRUE

	if(length(GLOB.dynamic_forced_roundstart_ruleset) > 0)
		rigged_roundstart()
	else
...
/datum/game_mode/dynamic/post_setup(report)
	...
	if(!GLOB.configuration.dynamic.no_intercept_report)
...
/datum/game_mode/dynamic/proc/rigged_roundstart()
	message_admins("[length(GLOB.dynamic_forced_roundstart_ruleset)] rulesets being forced. Will now attempt to draft players for them.")
	log_dynamic("[length(GLOB.dynamic_forced_roundstart_ruleset)] rulesets being forced. Will now attempt to draft players for them.")
...
/datum/game_mode/dynamic/proc/roundstart(list/roundstart_rules)
	...
	for(var/datum/dynamic_ruleset/roundstart/rule in roundstart_rules)
		...
		if(rule.acceptable(roundstart_pop_ready, threat_level) && round_start_budget >= rule.cost) // If we got the population and threat required
			rule.candidates = candidates.Copy()
			rule.trim_candidates()
			if(rule.ready(roundstart_pop_ready) && length(rule.candidates) > 0)
...
/datum/game_mode/dynamic/proc/check_blocking(list/blocking_list, list/rule_list)
	. = FALSE
	if(!length(blocking_list))
		return
	for(var/blocking in blocking_list)
		for(var/datum/executed as anything in rule_list)
...
/datum/game_mode/dynamic/make_antag_chance(mob/living/carbon/human/character)
	...
	if(forced_latejoin_rule)
		forced_latejoin_rule.candidates = list(character)
		...
		for (var/datum/dynamic_ruleset/latejoin/rule in latejoin_rules)
			if(!rule.weight)
				continue
			if(rule.acceptable(length(GLOB.alive_player_list), threat_level) && mid_round_budget >= rule.cost)
				// No stacking : only one round-ender, unless threat level > stacking_limit.
				if(threat_level < GLOB.dynamic_stacking_limit && GLOB.dynamic_no_stacking)
					if(rule.flags & HIGH_IMPACT_RULESET && high_impact_ruleset_executed)
						continue

				rule.candidates = list(character)
				rule.trim_candidates()
				if(rule.ready())
					drafted_rules[rule] = rule.get_weight()

		if(length(drafted_rules) > 0 && pick_latejoin_rule(drafted_rules))
			var/latejoin_injection_cooldown_middle = 0.5 * (latejoin_delay_max + latejoin_delay_min)
			latejoin_injection_cooldown = round(clamp(EXP_DISTRIBUTION(latejoin_injection_cooldown_middle), latejoin_delay_min, latejoin_delay_max)) + world.time
...
/datum/game_mode/dynamic/proc/configure_ruleset(datum/dynamic_ruleset/ruleset)
	...
	if(GLOB.configuration.dynamic.protect_roles_from_antagonist)
		ruleset.restricted_roles |= ruleset.protected_roles
	if(GLOB.configuration.dynamic.protect_assistant_from_antagonist)
