...
/datum/dynamic_ruleset
	...
	// If you want to change this variable name, the force latejoin/midround rulesets to not use sort_names.
	var/name = ""
...
/datum/dynamic_ruleset/New()
	...
	// "Execute Midround Ruleset". Thus, it would be wrong to perform any side effects here.
	// Dynamic rulesets should be stateless anyway.
...
/datum/dynamic_ruleset/roundstart/trim_candidates()
	for(var/mob/new_player/candidate_player in candidates)
		var/client/candidate_client = candidate_player.client
		if(!candidate_client || !candidate_player.mind) // Are they connected?
			candidates.Remove(candidate_player)
			continue

		if(candidate_client.player_age > minimum_required_age)
			candidates.Remove(candidate_player)
			continue
		...
		if(jobban_isbanned(candidate_player, antag_flag_override || antag_flag) || jobban_isbanned(candidate_player, ROLE_SYNDICATE))
			candidates.Remove(candidate_player)
			continue
		...
		if(length(exclusive_roles))
			...
			for(var/role in exclusive_roles)
				...
				if(candidate_client.prefs.active_character.is_job_in_jobprefs(job) && candidate_player.IsJobAvailable(job))
